		...
		return (
			<div>
				<form onSubmit={this.handleSubmit} class="form-inline">
					{cars.map(car => {
						const { modelCar, number, vin } = car;
						return (
							<div>
								<input onChange={this.handleModelreactChange} type="text" placeholder={`${modelCar}`} value={this.state.modelCar} name="modelCar" class="form-control mr-2 mb-2 " />
								<input onChange={this.handleNumberCarChange} type="text" placeholder={`${number}`} value={this.state.numberCar} name="number" class="form-control mr-2 mb-2 " />
								<input onChange={this.handleVinCarChange} type="text" placeholder={`${vin}`} value={this.state.vinCar} name="vinCar" class="form-control mr-2 mb-2" />
